...
func normalizeArchivePath(filePath string) string {
	return strings.TrimPrefix(filepath.ToSlash(filepath.Clean(filePath)), "./")
}
...
func (v *extraFilesProcessor) validate(name, src string) error {
	...
	if !v.cfg.SkipFilesValidation {
		if info.Size() > 20*1024 {
			return fmt.Errorf("extra file %s is larger than 20KB (%d bytes)", name, info.Size())
		}
		...
		if bytes.IndexByte(buf[:n], 0) != -1 {
			return fmt.Errorf("extra file %s appears to be a binary file", name)
		}
	}
	return nil
}
...
func (v *extraFilesProcessor) processStringArray(arr []string) []string {
	...
	for _, item := range arr {
		if _, ok := v.extraFiles[item]; ok {
			out = append(out, "${FILESDIR}/"+strings.TrimPrefix(item, "files/"))
		} else {
			out = append(out, item)
		}
	}
...
func gentooExtraFilePath(name string) (string, error) {
	...
	if strings.Contains(cleanName, "/") {
		return "", fmt.Errorf("extra file %q is in a subdirectory; extra files must be in the package root or files/", name)
	}
	return "files/" + cleanName, nil
}
