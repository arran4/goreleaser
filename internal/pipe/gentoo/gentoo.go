...
func (g *publishGroup) applyVersionRetention(ctx *context.Context, repoClient client.Client, repo client.Repo) ([]string, error) {
	...
	if len(ebuilds) == 0 {
		settings, err := loadOverlaySettings(ctx, g.cfg, repoClient, listRepo)
		if err == nil && !settings.thin {
			manifestPath := pathlib.Join(dir, "Manifest")
			manifestLines, err := loadManifestLines(ctx, repoClient, listRepo, manifestPath)
			if err == nil {
		...
		if dl, ok := repoClient.(client.FileDownloader); ok {
			for i := range g.files {
				...
				if maxR != -1 && maxREbuild != "" {
					existingEbuildContent, err := dl.DownloadFile(ctx, listRepo, pathlib.Join(dir, maxREbuild))
					if err == nil {
						...
						if !isDifferent {
							for _, f := range g.files {
								...
								existingContent, dErr := dl.DownloadFile(ctx, listRepo, f.Path)
