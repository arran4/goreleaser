...
func (Pipe) Publish(ctx *context.Context) error {
	...
	for _, g := range groups {
		...
		downloadRepo := repo
		if g.cfg.Repository.PullRequest.Enabled {
			downloadRepo.Branch = g.cfg.Repository.PullRequest.Base.Branch
		}

		if err := handleGentooManifestAndMetadata(ctx, g.cfg, repoClient, downloadRepo, &g.files, deletedEbuilds); err != nil {
			return err
		}
