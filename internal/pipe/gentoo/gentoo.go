...
func doRun(ctx *context.Context, cfg config.Gentoo, cl client.ReleaseURLTemplater) error {
	...
	ebuildPath, err := tp.Apply(cfg.Path)
	if err != nil {
		return err
	}

	ebuildPath = filepath.Join(ctx.Config.Dist, "gentoo", cfg.ID, ebuildPath)
	...
	log.WithField("ebuild", ebuildPath).Info("writing")
	if err := os.WriteFile(ebuildPath, []byte(content), 0o644); err != nil {
		return err
	}

	if err := ef.InstallExtraFiles(ctx, ebuildPath); err != nil {
		return err
	}

	ctx.Artifacts.Add(&artifact.Artifact{
		Name: filepath.Base(ebuildPath),
		Path: ebuildPath,
	...
	if cfg.MetaCache {
		pkgVer := strings.TrimSuffix(filepath.Base(ebuildPath), ".ebuild")
		category := strings.Split(filepath.ToSlash(filepath.Clean(cfg.Path)), "/")[0]
		metaCachePath := path.Join("metadata", "md5-cache", category, pkgVer)
...
func (g *publishGroup) applyVersionRetention(ctx *context.Context, repoClient client.Client, repo client.Repo) ([]string, error) {
	...
	if lister, ok := repoClient.(client.DirectoryLister); ok && g.cfg.MetaCache {
		cacheNames, err := lister.ListDir(ctx, listRepo, path.Join("metadata", "md5-cache", category))
...
func (g *publishGroup) publish(ctx *context.Context, cl client.Client) error {
	...
	if g.cfg.Repository.PullRequest.Enabled {
		...
		if fscli, ok := repoClient.(client.ForkSyncer); ok {
