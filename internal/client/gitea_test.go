...
func TestGiteaListDir(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		defer r.Body.Close()
		if r.URL.Path == "/api/v1/version" {
		...
		if r.URL.Path == "/api/v1/repos/someone/something/contents/app-category/foo" {
			w.WriteHeader(http.StatusOK)
			fmt.Fprint(w, `[{"name":"foo-1.0.ebuild","type":"file"},{"name":"subdir","type":"dir"}]`)
			return
		}
		if r.URL.Path == "/api/v1/repos/someone/something/contents/notfound" {
			w.WriteHeader(http.StatusNotFound)
			fmt.Fprint(w, `{"message":"not found"}`)
			return
		}
	...
	repo := Repo{Owner: "someone", Name: "something", Branch: "main"}

	names, err := client.ListDir(ctx, repo, "app-category/foo")
	require.NoError(t, err)
	require.Equal(t, []string{"foo-1.0.ebuild"}, names)

	names, err = client.ListDir(ctx, repo, "notfound")
	require.NoError(t, err)
	require.Nil(t, names)
}

func TestGiteaDeleteFile(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		defer r.Body.Close()
		if r.URL.Path == "/api/v1/version" {
		...
		if r.URL.Path == "/api/v1/repos/someone/something/contents/app-category/foo/foo-1.0.ebuild" {
			if r.Method == http.MethodGet {
				w.WriteHeader(http.StatusOK)
				fmt.Fprint(w, `{"name":"foo-1.0.ebuild","sha":"abc123sha"}`)
				return
			}
			if r.Method == http.MethodDelete {
				w.WriteHeader(http.StatusOK)
				fmt.Fprint(w, `{"content":null}`)
				return
			}
		}
		if r.URL.Path == "/api/v1/repos/someone/something/contents/notfound.txt" {
			w.WriteHeader(http.StatusNotFound)
			fmt.Fprint(w, `{"message":"not found"}`)
			return
		}
	...
	repo := Repo{Owner: "someone", Name: "something", Branch: "main"}
	author := config.CommitAuthor{Name: "Bot", Email: "bot@example.com"}

	err = client.DeleteFile(ctx, author, repo, "app-category/foo/foo-1.0.ebuild", "delete ebuild")
	require.NoError(t, err)

	err = client.DeleteFile(ctx, author, repo, "notfound.txt", "delete non-existent file")
	require.NoError(t, err)
}
