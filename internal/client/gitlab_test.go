...
func TestGitLabListDir(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		defer r.Body.Close()
		if strings.HasPrefix(r.URL.Path, "/api/v4/version") {
		...
		if strings.Contains(r.URL.Path, "/repository/tree") {
			if strings.Contains(r.URL.RawQuery, "path=app-category%2Ffoo") {
				w.WriteHeader(http.StatusOK)
				fmt.Fprint(w, `[{"name":"foo-1.0.ebuild","type":"blob"},{"name":"subdir","type":"tree"}]`)
				return
			}
			if strings.Contains(r.URL.RawQuery, "path=notfound") {
				w.WriteHeader(http.StatusNotFound)
				fmt.Fprint(w, `{"message":"404 Not Found"}`)
				return
			}
...
func TestGitLabDeleteFile(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		defer r.Body.Close()
		if strings.HasPrefix(r.URL.Path, "/api/v4/version") {
		...
		if strings.Contains(r.URL.Path, "/repository/files/app-category%2Ffoo%2Ffoo-1.0.ebuild") {
			w.WriteHeader(http.StatusOK)
			fmt.Fprint(w, `{"file_path":"app-category/foo/foo-1.0.ebuild","branch":"main"}`)
			return
		}
		if strings.Contains(r.URL.Path, "/repository/files/notfound.txt") {
			w.WriteHeader(http.StatusNotFound)
			fmt.Fprint(w, `{"message":"404 File Not Found"}`)
			return
		}
